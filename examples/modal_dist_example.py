...
from claster_quality.measures import compare_clusters
...
for column in data_df.columns:
    ...
    ax.scatter(centers, y, s=15)
    plt.title("Angular modal distribution {1}, classical and circular kmeans, k={0}".format(n_clusters,column))
    plt.show()
    #print("WCCSratio:",wccs_circ/wccs_euc)
    clusters[column] = {'basic':labels_basic, 'circ':labels_circ}
    #print(column)
...
for id_col in range(len(data_df.columns)):
    ...
    _res = {'angle1': column, 'angle2': column,  'type1': 'basic',  'type2': 'circ'}
